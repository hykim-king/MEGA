 ...
 * 
 * import com.pcwk.ehr.membership.domain.MembershipDTO; import
 ...
 * @WebAppConfiguration
 * 
 * @Import(MembershipControllerTest.TestStubConfig.class) class
 * MembershipControllerTest {
 * 
 * ------------------------------------------------- 테스트용 Stub Bean 모음
 * -------------------------------------------------
 * 
 * @Configuration static class TestStubConfig {
 * 
 * 가짜 FindIdService
 * 
 * @Bean public com.pcwk.ehr.login.service.FindIdService findIdService() {
 * return Mockito.mock(com.pcwk.ehr.login.service.FindIdService.class); }
 * 
 * 가짜 FindPwdService
 * 
 * @Bean public com.pcwk.ehr.login.service.FindPwdService findPwdService() {
 * return Mockito.mock(com.pcwk.ehr.login.service.FindPwdService.class); }
 * 
 * 가짜 LoginService ← 반드시 여기 안에!
 * 
 * @Bean public com.pcwk.ehr.login.service.LoginService loginService() { return
 * Mockito.mock(com.pcwk.ehr.login.service.LoginService.class); } } // ★
 ...
 * 
 * MockMvc mockMvc; MembershipDTO dto;
 * 
 * @BeforeEach void setup() throws Exception { mockMvc =
 * MockMvcBuilders.webAppContextSetup(ctx).build();
 * 
 * mapper.deleteAll(); dto = new MembershipDTO("pcwk01", "admin01",
 * "pcwk@naver.com", "pw1234", new Date(System.currentTimeMillis()), "N",
 * "token", 1, null, null, 0); mapper.doSave(dto); }
 * 
 *//** 목록 페이지 200 OK & 뷰 이름 */
/*
 * 
 * //@Disabled
 * 
 * @Test void listPage() throws Exception {
 * mockMvc.perform(get("/membership/doRetrieve.do") .param("pageNo", "1")
 * .param("pageSize", "10") .param("searchWord", ""))
 ...
 * @Disabled
 * 
 * @Test void detailPage() throws Exception {
 * mockMvc.perform(get("/membership/doSelectOne.do") .param("userId", "pcwk01"))
 * .andExpect(status().isOk())
 ...
 *//** 등록 후 리다이렉트 *//*
					 ...
					 * .contentType(MediaType.APPLICATION_FORM_URLENCODED) .param("userId",
					 * "test02") .param("email", "test02@naver.com") .param("password", "pw5678")
					 * .param("birth", "1990-01-01") .param("grade", "1"))
